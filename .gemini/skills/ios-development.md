...
- Use Apple documentation lookup before relying on memory for Apple APIs. Prefer:
  ...
  2. The `apple-docs` MCP tools for Swift, SwiftUI, UIKit, AppKit, and other Apple framework symbols.
  3. Official Apple Developer documentation in the browser if the MCP tools do not return enough detail.
- Use Context7 for non-Apple dependencies such as Firebase, Supabase, Stripe, Swift package libraries, backend SDKs, and JavaScript tooling around the app.
